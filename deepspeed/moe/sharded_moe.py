...
def top1gating(logits: Tensor,
    ...
    if used_token is not None:
        gates = einsum("s,se->se", used_token, gates)

    topk_score, topk_idx = torch.topk(
        gates.transpose(0, 1), k=capacity, dim=-1, largest=True
        # score, k=1, dim=-1, largest=True
    )  # [.. x top_k]

    # gating decisions
    exp_counts = torch.sum(gates > 0, dim=0).detach().to('cpu')

    # no aux loss for expert choice
    l_aux = 0.0

    # return l_aux, combine_weights, dispatch_mask, exp_counts
    return l_aux, topk_score, topk_idx, exp_counts
...
class MOELayer(Base):
    ...
    def forward(self, *input: Tensor, **kwargs: Any) -> Tensor:
        ...
        # TODO tutel doesn't work for expert choice
        assert not self.use_tutel, "tutel doesn't work for expert choice"
        if self.use_tutel:
